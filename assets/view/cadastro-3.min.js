...
if(getCookie("id") !== "" && getCookie("id") !== "0") {
            ...
            if (typeof e.target.files[0] !== "undefined") {
                ...
                if (/^image\//.test(file.type)) {
                    compressImage(file, 500, 500, webp("jpg"), function (resource) {
                        ...
                        localStorage.medico3 = JSON.stringify({"photo_64": resource});
                        $("#foto").val(resource);
    ...
    function avancar() {
        let medico3 = {"photo_64": $("#foto").val(), "termos": $("#termos").prop("checked")}
    ...
    function autoLogin(medico) {
        ...
        apiPost("login", {email: medico.email, password: medico.password}).then(g => {
            if (g) {
                ...
                pageTransition("login", "route", "forward");
    ...
    function validateMedico(medico) {
        ...
        if (!localStorage.medico) {
            ...
            toast("Passo 1 ausênte", 300, "toast-warning", function () {
                pageTransition("cadastro-usuario", "route", "back")
            })
        }
        if (!localStorage.medico2) {
            error = !0;
            toast("Passo 2 ausênte", 300, "toast-warning", function () {
                pageTransition("cadastro-2", "route", "back")
            })
        }
        return !error
    }
