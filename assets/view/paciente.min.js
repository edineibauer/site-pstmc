if (getCookie("id") === "" || getCookie("id") === "0" || isEmpty(history.state.param.id)) {
    ...
    function startCalendar() {
        ...
        $("#date-start").val("2019-10-15").trigger("change");
        // $("#date-end").val("2019-11-15").trigger("change");
    }
    ...
    function privateChartGetDataMakerXY(chart) {
        ...
        $.each(chart.data, function (i, e) {
            ...
            if (chartFilter.interval === "year" && chart.fieldDate) {
                ...
                let listDateinLabelX = ["name", "kilo_burn_activity", "FloorsClimbed", "runningMeters", "duration", "footsteps"];

                /**
                 * Se for filtro Dia e tiver data, então converte a label do eixo X em número de registros
                 */
                if(listDateinLabelX.indexOf(chart.fieldY) === -1) {
                    ...
                    chart.functionLabelX = x => {
                        return moment(chartFilter.dateStart).format("L");
                    };
                }

                let dateCheck = new RegExp("^\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}$", "i");
                if (!dateCheck.test(x) && !(chart.fieldY === "name" && chart.fieldX === "date"))
                    x += " " + zeroEsquerda(i) + ":00"
            }
            ...
            if (!isEmpty(dadosTabela)) {
                ...
                if (chart.fieldDate) {
                    if (chartFilter.interval === "week" || chartFilter.interval === "month") {
                         ...
                         * Ano
                         */
                        let dateStart = moment(chartFilter.dateStart).startOf('month');
                        let endOfMonth = moment(chartFilter.dateEnd).endOf('month');

                        /**
                         * Diminui os dias da semana na data inicial e adicionar na data final
                         * para fazer um calendário com os dias preenchidos
                         */
                        let formatDate = dateStart.format("YYYY-MM-DD");
                        let formatDateEnd = endOfMonth.format("YYYY-MM-DD");

                        while(formatDate !== formatDateEnd) {
                            let dateNow = dateStart.format('YYYY-MM') + "-15";
                            if (typeof dadosTabela[dateNow] === "undefined") {
                                if (chart.operacao === "sum" || chart.operacao === "media" || chart.operacao === "maioria")
                                    dadosTabela[dateNow] = null;
                                else
                            ...
                            formatDate = dateStart.format('YYYY-MM-DD');
                        }
                    }
                }
                ...
                if (isStringYLabel) {
                    ...
                    if(typeof chart.functionTooltips !== "function") {
    ...
    function privateChartGenerateOptions($this) {
        ...
        if (["radar", "pie", "doughnut", "polarArea"].indexOf($this.type[0]) > -1) {
            ...
            $this.backgroundColor = function (context) {
                ...
                let value = $this.functionAssocLabelY(data.y);
                return $this.functionColor(value, data.x, data.v, data.r)
            };
    ...
    function graficoSintomas(registrosBase) {
        ...
        for (let i in registrosBase) {
            registrosBase[i].created = registrosBase[i].created.split(" ")[0];
    ...
    function graficoMedicamentos(registrosBase) {
        ...
        if (modChart.medicamentos === 1) {
            let registrosCount = {};
            for(let i in registrosBase) {
                if(isInFilterDate(registrosBase[i].date) && typeof registrosBase[i].shouldTake !== "undefined" && !isNaN(registrosBase[i].shouldTake) && registrosBase[i].shouldTake > 0) {

                    let valor = (registrosBase[i].takedNum < 1 ? 0 : (registrosBase[i].takedNum > registrosBase[i].shouldTake ? 100 : parseInt((registrosBase[i].takedNum / registrosBase[i].shouldTake)*100)));
                    if(typeof registrosCount[registrosBase[i].date] === "undefined") {
                        registrosCount[registrosBase[i].date] = {
                            name: registrosBase[i].name,
                            total: 1,
                            soma: valor
                        }
                    } else {
                        registrosCount[registrosBase[i].date].total ++;
                        registrosCount[registrosBase[i].date].soma += valor;
                    }
                }
            }
            ...
            if(chartFilter.interval === "year") {
                ...
                let registrosCountMonth = {};
                for(let i in registrosCount) {
                    let month = i.split("-");
                    month = month[0] + "-" + month[1];

                    if (typeof registrosCountMonth[month] === "undefined") {
                        registrosCountMonth[month] = {
                            name: registrosCount[i].name,
                            total: 0,
                            soma: 0
                        };
                    }

                    registrosCountMonth[month].total++;
                    registrosCountMonth[month].soma += registrosCount[i].soma;
                }

                for(let i in registrosCountMonth) {
                    let registroDia = {
                        date: i + "-15",
                        name: registrosCountMonth[i].name,
                        valor: (registrosCountMonth[i].soma === 0 && registrosCountMonth[i].total > 0 ? 100 : (registrosCountMonth[i].total === 0 ? 0 : parseInt(registrosCountMonth[i].soma / registrosCountMonth[i].total)))
                    };

                    if(registroDia.valor > 0) {
                        for(let e = 0; e <= registroDia.valor; e++)
                ...
                for(let i in registrosCount) {
                    let registroDia = {
                        date: i,
                        name: registrosCount[i].name,
                        valor: (registrosCount[i].soma > 0 ? parseInt(registrosCount[i].soma / registrosCount[i].total) : 0)
                    };

                    /**
                     * Utiliza a contagem de registros no dia para informar quantos medicamentos deveria tomar
                     */
                    registros.push(registroDia);
                    if(registroDia.valor > 0) {
                        for(let i = 0; i < registroDia.valor; i++)
                            registros.push(registroDia);
                    }
                }
            }
            ...
            /**
             * Prepara dados para gráfico 2
             */
            let registrosC = {};
            for(let i in registrosBase) {
                if(isInFilterDate(registrosBase[i].date) && typeof registrosBase[i].shouldTake !== "undefined" && !isNaN(registrosBase[i].shouldTake) && registrosBase[i].shouldTake > 0) {
                    if(typeof registrosC[registrosBase[i].name] === "undefined") {
                        registrosC[registrosBase[i].name] = {
                            value: 0,
                            total: 0
                        };
                    }

                    registrosC[registrosBase[i].name].total ++;
                    registrosC[registrosBase[i].name].value += (registrosBase[i].shouldTake > 0 && registrosBase[i].takedNum < 1 ? 0 : (registrosBase[i].shouldTake <= 0 || registrosBase[i].takedNum >= registrosBase[i].shouldTake ? 100 : ((registrosBase[i].takedNum * 100) / registrosBase[i].shouldTake)));
                }
            }
             ...
             */
            for(let name in registrosC)
                registros.push({valor: (registrosC[name].total < 1 ? 100 : (registrosC[name].value > 0 ? parseInt(registrosC[name].value / registrosC[name].total) : 0)), name: name});
        }
        ...
        let grafico = new ChartMaker();
        grafico.setData(registros);
        grafico.setFieldY(modChart.medicamentos === 1 ? "name" : "valor");
        grafico.setFieldX(modChart.medicamentos === 1 ? "date" : "name");
        grafico.setMinY(0);
        grafico.setHideLineX();
        ...
        if (modChart.medicamentos === 1) {
            ...
            grafico.setFunctionTooltips(function (x, y, v) {
                if(v === 1)
                    return lang.usoMedicamentoNao;

                return (v - 1) + lang.usoMedicamentoSim;
            });
            ...
            grafico.setFunctionColor(function (name, y, v) {
                //v -1 === porcentagem de uso que deveria
                ...
                if (typeof name === "string") {
                    ...
                    color = nameMedicamentosColor[name]
                }


                if(v < 91) {
                    color = "#" + addHexColor(color.replace("#", ""), (v < 35 ? "252525" : (v < 70 ? "191919" : "101010")));
                }
    ...
    function graficoAtividade(registros) {
        ...
        for (let i in registros) {
            if(modChart['atividade-fisica'] === 3) {
                ...
                if (typeof registros[i].duration === "string" && registros[i].duration !== "" && registros[i].duration.indexOf(":") > 0) {
                    let p = registros[i].duration.split(":");
                    if (typeof p[1] !== "undefined" && !isNaN(p[0]) && !isNaN(p[1]))
                        registros[i].duration = parseFloat(parseFloat(p[0] + "." + ((p[1] * 100) / 60)).toFixed(2));
                    else
                        registros[i].duration = parseFloat(0);

                } else if(typeof registros[i].duration === "string" && registros[i].duration !== "" && !isNaN(registros[i].duration)) {
                    registros[i].duration = parseFloat(registros[i].duration);
        ...
        /**
         * Se for filtro por dia, então pega somente dados do dia e soma
         */
        if(chartFilter.interval === "day") {
            let nn = {};
            for (let i in registros) {
                if(registros[i].date === chartFilter.dateStart) {
                    if (isEmpty(nn)) {
                        nn = {
                            date: registros[i].date,
                            duration: (!isEmpty(registros[i].duration) && !isNaN(registros[i].duration) ? parseInt(registros[i].duration) : null),
                            footsteps: (!isEmpty(registros[i].footsteps) && !isNaN(registros[i].footsteps) ? parseInt(registros[i].footsteps) : null),
                            kilo_burn_activity: (!isEmpty(registros[i].kilo_burn_activity) && !isNaN(registros[i].kilo_burn_activity) ? parseInt(registros[i].kilo_burn_activity) : null),
                            runningMeters: (!isEmpty(registros[i].runningMeters) && !isNaN(registros[i].runningMeters) ? parseInt(registros[i].runningMeters) : null),
                            FloorsClimbed: (!isEmpty(registros[i].FloorsClimbed) && !isNaN(registros[i].FloorsClimbed) ? parseInt(registros[i].FloorsClimbed) : null)
                        };
                    } else {
                        if (!isEmpty(registros[i].duration) && !isNaN(registros[i].duration))
                            nn.duration = ((!isEmpty(nn.duration) && !isNaN(nn.duration) ? nn.duration : 0) + parseInt(registros[i].duration));

                        if (!isEmpty(registros[i].footsteps) && !isNaN(registros[i].footsteps))
                            nn.footsteps = ((!isEmpty(nn.footsteps) && !isNaN(nn.footsteps) ? nn.footsteps : 0) + parseInt(registros[i].footsteps));

                        if (!isEmpty(registros[i].kilo_burn_activity) && !isNaN(registros[i].kilo_burn_activity))
                            nn.kilo_burn_activity = ((!isEmpty(nn.kilo_burn_activity) && !isNaN(nn.kilo_burn_activity) ? nn.kilo_burn_activity : 0) + parseInt(registros[i].kilo_burn_activity));

                        if (!isEmpty(registros[i].runningMeters) && !isNaN(registros[i].runningMeters))
                            nn.runningMeters = ((!isEmpty(nn.runningMeters) && !isNaN(nn.runningMeters) ? nn.runningMeters : 0) + parseInt(registros[i].runningMeters));

                        if (!isEmpty(registros[i].FloorsClimbed) && !isNaN(registros[i].FloorsClimbed))
                            nn.FloorsClimbed = ((!isEmpty(nn.FloorsClimbed) && !isNaN(nn.FloorsClimbed) ? nn.FloorsClimbed : 0) + parseInt(registros[i].FloorsClimbed));
                    }
                }
            }
        ...
        if(chartFilter.interval === "year" || modChart['atividade-fisica'] === 5)
        ...
        if(modChart['atividade-fisica'] === 4) {
            grafico.setFunctionTooltips(function (x, y) {
                let hours = ("0" + (Math.floor(y / 60))).slice(-1);
                let minutes = zeroEsquerda(y % 60);
                return hours + ":" + minutes + " hr"
            });

            grafico.setFunctionLabelY(function (x, y) {
                let hours = ("0" + (Math.floor(x / 60))).slice(-1);
                let minutes = zeroEsquerda(x % 60);
                return hours + ":" + minutes + " hr"
            });
    ...
    function graficoCrises(registros) {
        ...
        if(chartFilter.interval === "day") {
            ...
            for (let i in data) {
                ...
                if(v === "" && !isEmpty(listXComentarios[data[i].x]))
                    v = 0;
        ...
        setTimeout(function () {
            /**
             * Organiza espaço das intensidades
             */
            let xSpace = 691;
            ...
            $(".grafico-crises-intensidade").find(".img-proporcion").css({
                ...
                "max-height": (widthList * percentWidth) + "px",
                margin: "0 " + (widthList * percentMargin) + "px"
            });

            /**
             * Organiza espaço dos comentários
             */
            xSpace = 691;
            widthList = xSpace / xComments.length;
            percentWidth = (widthList < 30 ? .8 : (widthList < 50 ? .76 : (widthList < 70 ? .66 : (widthList < 90 ? .5 : .4))));
            percentMargin = (widthList < 30 ? .1 : (widthList < 50 ? .12 : (widthList < 70 ? .17 : (widthList < 90 ? .25 : .3))));
            $(".grafico-crises-comentarios").find(".img-proporcion").css({
                width: (widthList * percentWidth) + "px",
                "max-height": (widthList * percentWidth) + "px",
                margin: "0 " + (widthList * percentMargin) + "px"
            })
    ...
    function privateChartGetDataCrisesCalendar(data) {
        ...
        if (typeof data === "object" && data.constructor === Object && !isEmpty(data)) {
             ...
             */
            let funcaoTooltips = (x, y) => { return (isEmpty(y) ? "" : (y < 2.6 ? lang.criseFraca : (y < 7.6 ? lang.criseMedia : lang.criseForte))); }
            let funcaoColor = y => { return (y < 2.6 ? "#FF6D6D" : (y < 7.6 ? "#CD3B3B" : "#6F0000")); }
            ...
            if(chartFilter.interval === "year") {
                ...
                let formatDateEnd = endOfMonth.format("YYYY-MM-DD");
                let mesData = [];
                let currentMes = dateStart.format("MMM");
                let soma = 0;
                let count = 0;

                while(formatDate !== formatDateEnd) {
                    let mes = dateStart.format("MMM");
                    if(currentMes !== mes) {
                        let media = soma > 0 ? soma/count : 0;
                        let mediaColor = "color: #FFF;background: " + (soma === 0 ? "transparent" : funcaoColor(media))
                        listX.push({mes: currentMes, media: mediaColor, mediaTitle: funcaoTooltips("", media), data: mesData});
                        mesData = [];
                        currentMes = mes;
                        count = 0;
                        soma = 0;
                    ...
                    if(typeof data[formatDate] !== "undefined") {
                        mesData.push({
                            ...
                            dia: parseInt(dateStart.format("D"))
                        })
                    } else {
                        mesData.push({
                            title: "",
                    ...
                    if(!isEmpty(data[formatDate])) {
                        soma += parseInt(data[formatDate]);
                        count++;
                    }
                 ...
                 * Mês, Semana e dia
                 */
                let dateStart = moment(dataKeys[0]).startOf('month');
                let endOfMonth = moment(dataKeys[0]).endOf('month');

                /**
                 * Diminui os dias da semana na data inicial e adicionar na data final
                 * para fazer um calendário com os dias preenchidos
                 */
                dateStart = dateStart.subtract((parseInt(dateStart.format("D")) - 1), 'days');
                let formatDate = dateStart.subtract(parseInt(dateStart.format("d")), 'days').format("YYYY-MM-DD");
                let formatDateEnd = endOfMonth.add(7 - parseInt(endOfMonth.format("d")), 'days').format("YYYY-MM-DD");
     ...
     * @returns {string|void}
     */
    function privateChartGetLabelCalendar(date) {

        if (chartFilter.interval === "day") {
            date = moment(chartFilter.dateStart);
            return date.format("DD") + " de " + date.format("MMMM");
        } else if (chartFilter.interval === "week") {

            date = moment(date);
            let semanaInicial = Math.ceil(date.date() / 7);
            let mesInicial = date.format("MMMM");
            date.add(6, "day");
            let semanaFinal = Math.ceil(date.date() / 7);
            let mesFinal = date.format("MMMM");
        ...
        let startYear = moment(chartFilter.dateStart).format("YYYY");
        let endYear = moment(chartFilter.dateEnd).format("YYYY");

        return startYear + (startYear !== endYear ? " e " + endYear : "");
    }
    ...
    function graficoCrises2(registrosBase) {
        ...
        if (chartFilter.interval === "year") {
            ...
            getRequest('public/tpl/chartCalendarBubble.mustache').then(templateCalendar => {
                ...
                let mesInicial = moment(chartFilter.dateStart).format("M");
                let mesFinal = moment(chartFilter.dateEnd).format("M");

                if(mesInicial !== mesFinal) {

                    /**
                     * Separa os registros em 2 categorias, o mês inicial e final
                     */
                    let registrosMesInicial = {};
                    let registrosMesFinal = {};
                    for(let i in registros) {
                        if(moment(i).format("M") === mesInicial)
                            registrosMesInicial[i] = registros[i];
                        else
                            registrosMesFinal[i] = registros[i];
                    }

                    let calendar1 = Mustache.render(templateCalendar, Object.assign({
                        x: privateChartGetDataCrisesCalendar(registrosMesInicial),
                        label: privateChartGetLabelCalendar(chartFilter.dateStart)
                    }, lang));

                    let calendar2 = Mustache.render(templateCalendar, Object.assign({
                        x: privateChartGetDataCrisesCalendar(registrosMesFinal),
                        label: privateChartGetLabelCalendar(chartFilter.dateEnd)
                    }, lang));
    ...
    function graficoSono(registrosBase) {
        ...
        if(chartFilter.interval === "day") {
            ...
            for(let i in registros) {
                if (registros[i].date === chartFilter.dateStart) {
                    if(modChart['sono'] === 2) {
                        if(isEmpty(nn)) {
                            nn = registros[i];
                        } else if(!isEmpty(registros[i].duration) && !isNaN(registros[i].duration)) {
                            nn.duration += parseFloat(registros[i].duration);
                        }
                    } else {
                        nn.push(registros[i]);
                    }
            ...
            registros = (modChart['sono'] === 2 ? [nn] : nn);
        }
                    ...
                    if (!isEmpty(paciente) && (typeof ind === "undefined" || ind === indicador)) {
                        ...
                        dbLocal.exeRead(indicador).then(g => {
                            if (isEmpty(g) || typeof readIndicador[indicador] === "undefined") {
                                 ...
                                 */
                                let data = {"patient_id": parseInt(paciente.id), "interval": "year", "intensity": 1, "doctor_id": parseInt(getCookie("id"))};
                                ...
                                apiPost(action[indicador], data).then(t => {
                                    if (t) {
                                        ...
                                        clearInterval(load);
                                        $grafico.html("<div class='col'><h3 class='padding-64 align-center font-bold font-xlarge color-text-gray'>" + lang.permissao + "</h3></div>");
                                    }
                                })
                            }
    ...
    function privateChartDateUpdateLimit(useStartDateInsteadDateEnd) {
        ...
        let limit = (chartFilter.interval === "year" ? 365 : (chartFilter.interval === "month" ? privateChartGetNumberDaysMonth(now.getMonth()) - 1 : (chartFilter.interval === "week" ? 6 : 0)));
