if (getCookie("id") === "" || getCookie("id") === "0") {
    ...
    function graficoAtividade(registros) {
        ...
        if(modChart['atividade-fisica'] === 4) {
            ...
            grafico.setFunctionTooltips(function (x, y) {
                ...
                if (y.length > 3)
                    y = [y.slice(0, y.length - 3), ".", y.slice(y.length - 3)].join('');
        ...
        if(chartFilter.interval === "day") {
            ...
            for (let i in data) {
                ...
                if(chartFilter.interval === "year") {
                    ...
                    for(let i in listXComentarios) {
                        if(typeof i === "string" && i.indexOf(mes) === 0 && typeof listXComentarios[i] === "string" && listXComentarios[i].length)
                            comentarios += "<b style='float:left; width: 100%;text-align: left;padding-bottom: 10px;'>" + moment(i).format("ll") + ":</b><div style='width:100%;float:left;padding:0 0 15px 30px;text-align:left'>" + listXComentarios[i] + "</div>";
                    }
    ...
    function graficoHumor(registrosBase) {
        ...
        for (let i in registrosBase) {
            if (isInFilterDate(registrosBase[i].date))
                registros.push(registrosBase[i])
        }
