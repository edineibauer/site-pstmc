if (getCookie("id") === "" || getCookie("id") === "0") {
            ...
            getChart: type => {
                ...
                if (isEmpty($this.data)) {
                    ...
                    setTimeout(function () {
                        new Chart(ctx, {
                            type: $this.type[0],
                            data: {labels: $this.labels, datasets: getDataSets($this)},
    ...
    function graficoMedicamentos(registrosBase) {
        ...
        if (modChart.medicamentos === 1) {
            ...
            setTimeout(function () {
                $("#grafico-header-title-medicamentos").html(Mustache.render(tpl.graficoHeaderSubtitle, {title: relTitleMedicamentos, valor: bigger.v + "%", descricao: bigger.name + " " + lang.noPeriodoMedicamentos}));
            }, 1);
    ...
    function graficoAtividade(registros) {
        ...
        if (data.length) {
             ...
             */
            setTimeout(function () {
                $("#grafico-header-title-atividade-fisica").html(Mustache.render(tpl.graficoHeaderSubtitle, {title: relTitleAtividade, valor: media, descricao: getTitleIndicador("atividade-fisica") + " " + lang.noPeriodoAtividade}));
            }, 1)
        }
    ...
    function graficoCrises(registros) {
        ...
        for (let i in data) {
            ...
            if (typeof listXComentarios[data[i].x] === "string" && listXComentarios[data[i].x].length)
                xComments.push("<b>" + moment(data[i].x).format("ll") + ":</b> " + listXComentarios[data[i].x]); else xComments.push("")
        }
    ...
    function privateChartGetDataCrisesCalendar(registros, isPrevius) {
        ...
        /**
         * Obtém os dados default do gráfico para trabalhar com eles
         */
        let listX = [];
        let data = grafico.getData();
        data = chartDataOrder(data, "x").reverse();
    ...
    function graficoCrises2(registros) {
        ...
        if (chartFilter.interval === "year") {
            getRequest('public/tpl/chartCalendarYearBubble.mustache').then(tpl => {
                $content.append(Mustache.render(tpl, Object.assign({
            ...
            getRequest('public/tpl/chartCalendarBubble.mustache').then(ccc => {
                let calendar2 = Mustache.render(ccc, Object.assign({
                    x: privateChartGetDataCrisesCalendar(registros),
                    label: privateChartGetLabelCalendar()
                }, lang));

                let calendar1 = Mustache.render(ccc, Object.assign({
                    x: privateChartGetDataCrisesCalendar(registros, 1),
                    label: privateChartGetLabelCalendar(1)
                }, lang));

                getRequest('public/tpl/graficoCrisesCalendar.mustache').then(crisesCalendar => {
                    $content.append(Mustache.render(crisesCalendar, Object.assign({
                        calendar1: calendar1,
                        calendar2: calendar2
                    }, lang)));
                });
    ...
    function graficoSono(registrosBase) {
        ...
        if (modChart.sono === 1) {
            ...
            if (!isNaN(media))
                media = Math.floor(media) + ":" + Math.round(media % 1 * 60); else media = 0;
            ...
            let graficoSono = {media: parseInt((bom * 100) / data.length)};
