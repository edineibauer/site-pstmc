if (getCookie("id") === "" || getCookie("id") === "0") {
    ...
    function setDataperfil() {
        setTimeout(function () {
            ...
            else if (localStorage.photo_64 && localStorage.photo_64 !== "null" && !isEmpty(localStorage.photo_64))
                $("#cadastro-image2").html("<img src='" + localStorage.photo_64 + "' width='150' height='150' style='height: 150px;width: 150px;' />");
    ...
    function salvarPerfil() {
        ...
        if (validateMedico(medico)) {
            ...
            apiPost("update", medico).then(g => {
                if (g) {
                    setCookie("nome", medico.name);
                    setCookie("imagem", typeof medico.imagem === "object" && typeof medico.imagem[0] !== "undefined" ? medico.imagem[0].image : "");
                    setCookie("nome_usuario", slug(medico.name));
                    setCookie("email", medico.email);
    ...
    $(function () {
        ...
        getJSON('public/language/' + LANG + '/ajustes.json').then(lang => {
            getRequest('public/tpl/ajustes.mustache').then(tpl => {
                $("#perfil").append(Mustache.render(tpl, lang))
            })
        });
        setDataperfil()
    })
