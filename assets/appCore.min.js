/**
 * Script Base com as principais bibliotecas inclusas
 * este script é padrão e usado em todas as páginas
 *
 * composição deste js:
 *  - jquery [linha: 16]
 *  - Hammer [linha: 2619]
 *  - moment [linha: 3367]
 *  - toast (próprio: exibição de mensagens/notificações) [linha: 4921]
 *  - mustache [linha: 5016]
 *  - idb [linha: 5413]
 *  - indexedDB (próprio: camada para facilitar o uso do idb) [linha: 5638]
 *  - appCore (próprio: base para fazer route das páginas e controller do conteúdo) [linha: 6142]
 */
...
/**
 * jQuery Plugins desenvolvidos
 */
$(function ($) {

    /**
     * Adiciona 3 linhas de loading no topo do elemento alvo
     * @returns {jQuery}
     */
    $.fn.loading = function () {
...
function setUpdateVersion() {
    $.ajax({
        type: "POST", url: 'set', data: {lib: 'config', file: 'update', update: !0}, success: function (data) {
...
function post(lib, file, param, funcao) {
    ...
    $.ajax({
        type: "POST", url: 'set', data: convertEmptyArrayToNull(param), success: function (data) {
            if (data.response === 1) {
...
function get(file, retrying) {
    return getJSON("get/" + file).then(data => {
        if (data.response === 1) {
...
function view(file, funcao) {
    getJSON('public/language/' + LANG + '/' + file + ".json").then(lang => {
        getRequest('view/' + file + '.html').then(r => {
            funcao({content: Mustache.render(r, lang)});
        });
    });
}
...
function menuHeader() {
    ...
    loginBtn();
    return dbLocal.exeRead("__menu", 1).then(menu => {
        if (typeof menu !== "object" || menu.constructor !== Array || menu === null)
            menu = [];

        return getRequest('public/tpl/menu-header.mustache').then(tpl => {
            let content = "";
            for (let m in menu) {
                if (typeof menu[m].html === "string" && menu[m].html !== "undefined" && !isEmpty(menu[m].html))
                    content += Mustache.render(tpl, menu[m])
            }
...
function loadSyncNotSaved() {
    return new Promise((resolve, f) => {
        $.ajax({
            type: "GET", url: 'get/load/sync', success: function (data) {
                if (data.response === 1) {
                    let sync = data.data;
...
function clearCacheUser() {
    ...
    clear.push(dbLocal.clear("crises"));
    clear.push(dbLocal.clear("sintomas"));
    clear.push(dbLocal.clear("medicamentos"));
    clear.push(dbLocal.clear("atividade-fisica"));
    clear.push(dbLocal.clear("sono"));
    clear.push(dbLocal.clear("humor"));
        ...
        return clearCacheUser().then(() => {
            $.each(user, function (i, e) {
                if (i !== "id_old" && i !== "db_action" && i !== "db_status") {
                    ...
                    if (i === "birthday") {
                        let bb = e.replace('-', '/').replace('-', '/').split("/");
                        e = bb[2] + "-" + bb[1] + "-" + bb[0];
                    }

                    if (i === "photo_64") {
                        setCookie("imagem", e);
                        localStorage.setItem("imagem", e)
                    }
...
function loadCacheUser() {
    if (navigator.onLine) {
        let gets = [];
        let creates = [];
        gets.push(getJSON("public/language/" + LANG + "/menu.json"));
        gets.push(getJSON("get/menu.json"));
        gets.push(getJSON("get/templates.json"));
        return Promise.all(gets).then(r => {
            for (let i in r[1])
                r[1][i].html = Mustache.render(r[1][i].html, r[0]);

            creates.push(dbLocal.exeCreate('__menu', r[1]));
...
function updateTemplates() {
    return getJSON("get/templates.json").then(tpl => {
...
function checkMenuActive() {
    $(".menu-li").removeClass("active").each(function (i, e) {
        if ($(e).attr("rel") === app.file || $(e).find("[rel='" + app.file + "']").length || $(e).attr("rel") === app.file + "s")
            $(e).addClass("active")
    })
}
...
function animateTimeout($element, $aux, scroll) {
    ...
    if (window.innerWidth < 900 && $("#core-header-nav-bottom").css("opacity") !== "0" && $("#core-header-nav-bottom").hasClass("s-show"))
        $("#core-content").addClass("mb-50"); else $("#core-content").removeClass("mb-50")
}
...
var app = {
    file: "", route: "", loading: !1, removeLoading: function () {
        app.loading = !1;
        ...
        return view(file, function (g) {
            if (g) {
                getJSON("public/param/" + file + ".json").then(param => {
                    let baseParam = getBaseParam();
                    param = Object.assign({}, getBaseParam(), param, g);

                    if (app.haveAccessPermission(param.setor, g["!setor"])) {

                        headerShow(param.header);
                        $("#core-title").text(param.title);
                        getRequest("assets/view/" + file + ".min.css").then(css => {
                            $div.prepend("<style class='core-style'>" + css + (param.header ? "#core-content { margin-top: " + $("#core-header")[0].clientHeight + "px }" : "#core-content { margin-top: 0}") + "</style>");
                            $div.append(param.content);
                        });

                        if (!param.header)
                            $div.addClass("notop");

                        let topHeader = param.header ? $("#core-header")[0].clientHeight : 0;
                        $div.css("min-height", (window.innerHeight - topHeader - (window.innerWidth < 900 && param.navbar && $("#core-header-nav-bottom").hasClass("s-show") ? 50 : 0)) + "px");

                        $.cachedScript("assets/view/" + file + ".min.js").then(() => {
                            app.removeLoading()
                        }).catch(() => {
                            app.removeLoading()
                        });
...
function pageTransition(route, type, animation, target, param, scroll, setHistory) {
    ...
    route = (typeof route === "string" ? route : location.pathname).replace(".html", "");
    ...
    if (!app.loading && !aniTransitionPage) {
        ...
        if (history.length === 1 || !history.state)
            history.replaceState({
                ...
                param: {},
                scroll: scroll
            }, null, app.route + ".html"); else if (setHistory)
            history.replaceState({
                id: history.state.id,
        ...
        return Promise.all([]).then(() => {
            ...
            if (type === 'route') {
                ...
                let store = typeof param.store === "undefined" || ["false", "0", 0, !1].indexOf(param.store) === -1 ? 1 : 0;
                let data = (typeof param === "object" && typeof param.data === "object" && !isEmpty(param.data) ? param.data : {});
                if (!isEmpty(id))
                    data.id = id; else if (!isEmpty(data.id))
                    id = parseInt(data.id);
                let identificador = "";
                if (typeof param === "object" && typeof param.identificador === "string") {
                    ...
                    history.replaceState(history.state, null, app.route + ".html")
...
$(function () {
    ...
    /**
     * Intercepta a ação de navegação do navegador
     * @param event
     */
    ...
    /**
     * Intercepta os cliques realizados na página para atuar a transição de tela
     */
    $("body").off("click", "a").on("click", "a", function (e) {
        let url = $(this).attr("href").replace(".html", "");
        if (url === "#back") {
            e.preventDefault();
            history.back()
        } else {
            ...
            let param = $(this).hasAttr("data-id") ? {id: parseInt($(this).attr("data-id"))} : {};
            let p = new RegExp(/^#/i);
            let pjs = new RegExp(/^javascript/i);
            if ($(this).attr("target") !== "_blank" && !p.test(url) && !pjs.test(url)) {
                e.preventDefault();
                pageTransition(url, route, animation, target, param)
            }
        }

        /**
         * Remove o envio padrão dos formulários
         */
    ...
    checkSessao().then(() => {
        ...
        return Promise.all(t).then(cache => {
            ...
            return new Promise((resolve, reject) => {
                $.cachedScript("assets/core.min.js", {
                    success: function () {
                        resolve(1)
                    }, fail: function () {
                        reject(0)
