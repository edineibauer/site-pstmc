/**
 * Core é carregado após o conteudo da página ter sido renderizada
 *
 * composição deste js:
 * - Mask.js [linha: 13]
 * - Chart.js [linha: 301]
 * - chartjs-plugin-labels.js [linha: 4822]
 * - core.js (próprio: funções genéricas) [linha: 5097]
 * - intlTelInput [linha: 5395]
 */
...
(function () {
    ...
    if ("undefined" === typeof Chart) console.error("Can not find Chart object."); else {
        "function" != typeof Object.assign && (Object.assign = function (a, c) {
            ...
            for (var b = Object(a), e = 1; e < arguments.length; e++) {
                var d = arguments[e];
                if (null != d) for (var g in d) Object.prototype.hasOwnProperty.call(d, g) && (b[g] = d[g])
            }
            return b
        });
        var k = {};
        ...
        f.prototype.renderToDataset = function (a, c) {
            ...
            this.total = null;
            var b = this.args[c];
            b.meta.data.forEach(function (c, d) {
        ...
        f.prototype.renderLabel = function (a, c) {
            return this.options.arc ? this.renderArcLabel(a, c) : this.renderBaseLabel(a, c)
        };
        f.prototype.renderBaseLabel = function (a, c) {
            var b = this.ctx;
            if ("object" === typeof a) b.drawImage(a, c.x - a.width / 2, c.y - a.height / 2, a.width, a.height); else {
                b.save();
                b.textBaseline = "top";
                b.textAlign = "center";
                ...
                for (var e = a.split("\n"), d = 0; d < e.length; d++) b.fillText(e[d], c.x, c.y - this.options.fontSize / 2 * e.length + this.options.fontSize * d);
                b.restore()
            }
        };
        f.prototype.renderArcLabel = function (a, c) {
            var b = this.ctx, e = c.radius, d = c.view;
            b.save();
            b.translate(d.x, d.y);
            if ("string" === typeof a) {
                ...
                for (h = 0; h < d.length; ++h) {
                    var n = d[h], k = (d.length - 1 - h) * -this.options.fontSize + f;
                    b.save();
                    b.rotate((g - l[h]) / 2 / e);
            ...
            } else b.rotate((d.startAngle + Math.PI / 2 + c.endAngle) / 2), b.translate(0, -1 * e), this.renderLabel(a, {
                x: 0,
                y: 0
            });
            b.restore()
        ...
        f.prototype.shouldRenderToElement = function (a, c) {
            return !a.hidden && !c.hidden && (this.options.showZero || "polarArea" === this.chart.config.type ? 0 !== c._view.outerRadius : 0 !== c._view.circumference)
        };
        f.prototype.getLabel = function (a, c, b) {
            if ("function" === typeof this.options.render) a = this.options.render({
                ...
                case "label":
                    ...
                    break;
                case "image":
                    a = this.options.images[b] ? this.loadImage(this.options.images[b]) : "";
                    break;
                default:
                    a = this.getPercentage(a, c, b) + "%"
            }
            "object" === typeof a ? a = this.loadImage(a) : null !== a && void 0 !== a && (a = a.toString());
            return a
        };
        f.prototype.getFontColor = function (a, c, b) {
            ...
            "function" === typeof e ? e = e({
                ...
                dataset: a,
                index: b
            }) : "string" !== typeof e && (e = e[b] || this.chart.config.options.defaultFontColor);
            return e
        };
        f.prototype.getPercentage = function (a, c, b) {
            ...
            if ("polarArea" === this.chart.config.type) {
                if (null === this.total) for (c = this.total = 0; c < a.data.length; ++c) this.total += a.data[c];
            ...
            } else a = c._view.circumference / this.chart.config.options.circumference * 100;
            a = parseFloat(a.toFixed(this.options.precision));
            this.options.showActualPercentages || ("bar" === this.chart.config.type && (this.totalPercentage = this.barTotalPercentage[b] || 0), this.totalPercentage += a, 100 < this.totalPercentage && (a -= this.totalPercentage - 100, a = parseFloat(a.toFixed(this.options.precision))), "bar" === this.chart.config.type && (this.barTotalPercentage[b] = this.totalPercentage));
            return this.percentage = a
        };
        f.prototype.getRenderInfo = function (a, c) {
            return "bar" === this.chart.config.type ? this.getBarRenderInfo(a, c) : this.options.arc ? this.getArcRenderInfo(a, c) : this.getBaseRenderInfo(a, c)
        };
        f.prototype.getBaseRenderInfo = function (a, c) {
            if ("outside" === this.options.position || "border" === this.options.position) {
                var b, e = a._view, d = e.startAngle + (e.endAngle - e.startAngle) / 2, g = e.outerRadius / 2;
                "border" === this.options.position ? b = (e.outerRadius - g) / 2 + g : "outside" === this.options.position && (b = e.outerRadius - g + g + this.options.textMargin);
                b = {x: e.x + Math.cos(d) * b, y: e.y + Math.sin(d) * b};
                "outside" === this.options.position && (d = this.options.textMargin + this.measureLabel(c).width / 2, b.x += b.x < e.x ? -d : d);
                return b
            }
        ...
        f.prototype.getArcRenderInfo = function (a, c) {
            var b = a._view;
            var e = "outside" === this.options.position ? b.outerRadius + this.options.fontSize + this.options.textMargin : "border" === this.options.position ? (b.outerRadius / 2 + b.outerRadius) / 2 : (b.innerRadius + b.outerRadius) / 2;
            var d = b.startAngle, g = b.endAngle, l = g - d;
            d += Math.PI / 2;
            g += Math.PI / 2;
        ...
        f.prototype.drawable = function (a, c, b) {
            ...
            c = b.x - e.width / 2;
            var d = b.x + e.width / 2, g = b.y - e.height / 2;
            b = b.y + e.height / 2;
            return "outside" === this.options.renderInfo ? this.outsideInRange(c, d, g, b) : a.inRange(c, g) && a.inRange(c, b) && a.inRange(d, g) && a.inRange(d, b)
        };
        f.prototype.outsideInRange = function (a, c, b, e) {
            for (var d = this.labelBounds, g = 0; g < d.length; ++g) {
                for (var f = d[g], k = [[a, b], [a, e], [c, b], [c, e]], h = 0; h < k.length; ++h) {
                    var m = k[h][0], n = k[h][1];
                    if (m >= f.left && m <= f.right && n >= f.top && n <= f.bottom) return !1
                }
                k = [[f.left, f.top], [f.left, f.bottom], [f.right, f.top], [f.right, f.bottom]];
                for (h = 0; h < k.length; ++h) if (m = k[h][0], n = k[h][1], m >= a && m <= c && n >= b && n <= e) return !1
            }
...
/**
 * Define variável Mask para telefone
 */
...
function updatePerfilPage() {
    if (localStorage.cpf) {
        $(".perfil-photo_64, #core-header-perfil > img").attr("src", (!localStorage.photo_64 || localStorage.photo_64 === "null" || localStorage.photo_64 === "" ? "public/assets/img/foto-padrao.png" : localStorage.photo_64));
    ...
    $("#core-sidebar-menu").html("<a href='dashboard' class='menu-sidebar-li'>Início</a>").append("<a href='pacientes' class='menu-sidebar-li'>Pacientes</a>").append("<a href='perfil' class='menu-sidebar-li'>Seu Perfil</a>").append("<div onclick='openAjustes();closeSidebar();' class='menu-sidebar-li'>Ajustes</div>")
}
...
function readPaciente(id, retry) {
    return dbLocal.exeRead("pacientes", parseInt(id)).then(d => {
        if (!isEmpty(d)) {
            return d
            ...
            return pageTransition("pacientes", "route", "forward")
        }
    })
}

function getPacientesServer() {
    return apiPost("getPatientList", {"doctor_id": parseInt(getCookie("id"))}).then(p => {
        let aa = [];
        let pp = [];
        aa.push(p);
        if (!isEmpty(p)) {
            $.each(p, function (i, e) {
                ...
                e.patient.imagem = (typeof e.patient.photo_64 !== "undefined" && e.patient.photo_64 !== "null" && !isEmpty(e.patient.photo_64) ? e.patient.photo_64 : "public/assets/img/foto-padrao.png");
                pp.push(e.patient);
...
function clearFadeIn(content) {
    $(content).find(".easefadein").addClass("notransition").css("opacity", 0).css("transform", "translateY(-20px)").removeClass("notransition")
}
...
function closeAjustes() {
    let lang = $(".select-pais").val();
    if(!isEmpty(lang) && lang !== getCookie("LANG")) {
        setCookie("LANG", $(".select-pais").val());
        location.reload();
    }
...
/**
 * intlTelInput.js
 * biblioteca para o select box bandeira
 */
!function (a) {
